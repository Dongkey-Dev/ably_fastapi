...
from app.models import UserToken, UserInquireOut, UserPhoneToken, UserPhonePswdToken
...
invalid_phone_match_exception = getHTTPException("Token, phone are not matched.")
...
async def get_phone_hashpswd_token(payload : dict = Depends(valid_token)):
    try:
        ...
        if phone is None or doublehash_pswd is None:
            raise credentials_exception
        token_data = UserPhonePswdToken(phone=phone, doublehash_pswd=doublehash_pswd)
    except Exception as e:
        raise credentials_exception
    return token_data        
